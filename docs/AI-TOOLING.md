...
This project uses three local, free tools. They solve **two different problems** —
don't conflate them.
...
All three outputs are gitignored: the index DBs are large and regenerable; the vault
holds personal job data and must stay local (project constraint: *keep all data local*).
What **is** committed is the *code and instructions* that make them work.

---

## GitNexus + Graphify — code intelligence for the AI

These exist so an assistant working on the **code** can reason about impact and structure
instead of guessing. Usage is documented in `AGENTS.md` (GitNexus) and `CLAUDE.md` (Graphify).
...
The vault only becomes useful when notes are **linked**. A pile of 150 unconnected
notes produces a graph of disconnected dots (which is why it looked pointless). The
exporter fixes that by turning your job database into linked notes:
...
Because each job note wikilinks `[[company]]` and each skill, Obsidian's **graph view**
now surfaces companies and skills as **hub nodes** — you can see at a glance which
companies you're chasing, which skills recur, and what's `NEEDS_MANUAL` vs `PACKET_READY`.
...
4. Re-run `obsidian-sync` anytime; notes are overwritten in place (stable filenames).

A manual job-note template lives at `second-brain/_templates/job.md` for notes you write
by hand (kept local). Configure a different vault location via `obsidian_vault_dir` in
`config.json` or the `--vault` flag.
