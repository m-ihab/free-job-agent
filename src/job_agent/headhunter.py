...
class OutreachPack:
    ...
    def to_markdown(self) -> str:
        lines = [
            ...
            f"---",
            f"",
            f"## LinkedIn Connection Request",
            f"*(250 char limit — copy & paste into LinkedIn)*",
            f"",
            f"```",
            self.connect_request,
            f"```",
            f"",
            f"## LinkedIn Recruiter Message",
            f"*(Send after connecting, or via InMail)*",
            f"",
            self.recruiter_message,
            f"",
            f"## Follow-Up LinkedIn Message",
            f"*(Send ~7 days after applying if no response)*",
            f"",
