"""Render tailored CV content to LaTeX.
...
:mod:`job_agent.renderer.latex_helpers`; image/asset handling in
:mod:`job_agent.renderer.latex_assets`; and compilation in
:mod:`job_agent.renderer.latex_compile`. All are re-exported here so existing
import paths (``from job_agent.renderer.latex_render import ...``) keep working.
"""
...
# Re-exported for callers/tests that import these from this module's historical
# path (``from job_agent.renderer.latex_render import ...``).
from job_agent.renderer.latex_helpers import (  # noqa: F401  (re-export seam)
    _clean_role_phrase,
    _detect_contract_family,
)
from job_agent.renderer.latex_assets import (  # noqa: F401  (re-export seam)
    _image_present,
    copy_latex_assets,
    neutralize_missing_images,
)
from job_agent.renderer.latex_compile import (  # noqa: F401  (re-export seam)
    LatexCompileError,
...
__all__ = [
    # rendering / language (defined here)
    "set_cvlang",
    "detect_cvlang",
    "compact_cv_source",
    "render_moderncv_template",
    "render_latex_source",
    # re-exported compile subsystem
    "LatexCompileError",
    "available_latex_compiler",
    "compile_latex_to_pdf",
    "count_pdf_pages",
    # re-exported asset handling
    "copy_latex_assets",
    "neutralize_missing_images",
    "_image_present",
    # re-exported text helpers
    "_tailored_summary",
    "_clean_role_phrase",
    "_detect_contract_family",
]
